...
export default function SimpleCourseFormPage() {
  ...
  const handleAddTopic = () => {
    ...
    if (editingIndex !== null) {
      ...
      updatedTopics[editingIndex] = { ...updatedTopics[editingIndex], ...newTopic };
  ...
  const handleEditTopic = (index) => {
    ...
    window.scrollTo({ top: document.getElementById('topic-editor').offsetTop - 100, behavior: 'smooth' });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const courseData = {
        ...
        slug: courseInfo.title.toLowerCase().replace(/[^a-z0-9]+/g, '-') + '-' + Date.now().toString().slice(-6),
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-2xl font-bold text-gray-900">
                {isEdit ? 'Edit Course' : 'Create Course (Simple)'}
              </h1>
            </div>
            <div className="flex items-center space-x-4">
